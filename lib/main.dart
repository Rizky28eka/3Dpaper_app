...
import 'package:paper_3d_app/camera/camera_state_machine.dart';
  ...
  Widget build(BuildContext context) {
    return MaterialApp(
        ...
        home: BlocProvider<CameraCubit>(
            create: (context) => CameraCubit(), child: const BoxWorld()));
  }
}
...
class BoxWorld extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final BehaviorSubject<Size> screenSubject =
        BehaviorSubject<Size>.seeded(screenSize.size);

    final cameraCubit = context.read<CameraCubit>();
    final animationStateMachine =
        CameraAnimationStateMachine(cubitToSubject(cameraCubit), tickerManager);
